...
import { readExploreFilters, readPage, readUiLocale } from "~/lib/explore";
...
export async function loadExplore(request: Request, scope?: { kind: TaxonomyKind; slug: string }) {
  ...
  if (!scope) {
    ...
    if (kind) {
      ...
      url.pathname = "/" + kind + "/" + encodeURIComponent(identity.slug);
  ...
  if (scope) {
    ...
    taxonomy = identity;
    url.pathname = "/" + scope.kind + "/" + encodeURIComponent(identity.slug);
  ...
  if (url.searchParams.get("content_type") !== "image") url.searchParams.delete("content_type");
